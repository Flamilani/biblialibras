<div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
      <h3 class="page-header"><a href="<?php echo base_url('admin/planos'); ?>"> <i class="fa fa-file fa-fw"></i> Planos</a> > Períodos do <?php echo $nome_plano[0]->nome_plano; ?> </h3>
            ...
            <!-- /.row -->
             <?php if($this->session->flashdata('success')) { ?>
                <div class="alert alert-success alert-dismissible">
                    ...
                    <i class="icon fa fa-success"></i>O período foi adicionado com sucesso!</div>
            <?php } else if($this->session->flashdata('deletar')) { ?>
                   <div class="alert alert-danger alert-dismissible">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <i class="icon fa fa-success"></i>O período foi deletado com sucesso!</div>
           <?php } ?>
            ...
            <div class="row">
                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading">
                          <button class="btn btn-info" id="cadastro_livro">Cadastrar Período</button>
                        </div>
  ...
  $duracao = array('name' => 'duracao', 'id' => 'duracao', 'type' => 'number', 'value' => set_value('duracao'), 'class' => 'form-control', 'placeholder' => '0');  
   ...
   $ordem = array('name' => 'ordem', 'type' => 'number', 'id' => 'ordem', 'value' => '000', 'class' => 'form-control', 'placeholder' => '000');
     ...
     $buttonPubl = array('name' => 'btn_publicar', 'id' => 'btn_publicar', 'type' => 'submit', 'class' => 'btn btn-success', 'value' => 'Publicar'); ?>
            ...
            <?php echo form_open_multipart('admin/planos/adicionar_periodos', 'role="form"'); ?>
      ...
      <?php form_close(); ?>          
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                           <?php echo $count_periodo_p; ?> <?php plural($count_periodo_p, 'Período', 'Períodos') ?>
                        </div>
                        <!-- /.panel-heading -->
            ...
            <table width="100%" class="tabela1 table table-striped table-bordered table-hover" id="tabela1">
                            ...
                            <?php foreach ($periodos as $periodo): ?>
                                <tr class="odd gradeX">                               
       <td class="text-center"><?php echo form_hidden($periodo->id_plano); ?><?php echo $periodo->ordem; ?></td>
                                       <td><?php echo $periodo->titulo; ?> ( <?php echo $periodo->duracao; ?> <?php echo FormPeriodo($periodo->periodo); ?>)</td>     
                                        <td class="text-center"><?php echo reais($periodo->valor); ?></td>  
                                          <td class="text-center">
                                              <?php echo $periodo->duracao; ?> <?php echo FormPeriodo($periodo->periodo); ?>
                                          </td>
                                            <td class="text-center">
       <b data="<?php echo $periodo->id_plano; ?>" class="status_checks btn btn-sm <?php echo ($periodo->status) ? 'btn-success' : 'btn-warning' ?>"><?php echo ($periodo->status) ? 'Ativo <span title="Ativo" class="glyphicon glyphicon-ok"></span>' : 'Inativo <span title="Inativo" class="glyphicon glyphicon-minus"></span>' ?></b>                                            
                                          </td>
                                        <td class="text-center">
            <a href="<?php echo base_url('admin/planos/alterar_periodo/' . $periodo->id_plano); ?>" title="Editar" class="btn btn-sm btn-primary"><b><i class="fa fa-edit"></i></b></a>

  <a href="<?php echo base_url('admin/planos/deletar/' . $periodo->id_plano); ?>" onclick="return confirmarExclusao(<?php echo $periodo->id_plano; ?>)" title="Deletar" class="btn btn-sm btn-danger"><b><i class="fa fa-trash-o"></i></b></a>
                                      </td>
                                    </tr>   
                                         <?php endforeach; ?>
                                      <?php endif; ?>                
    ...
    $('#valor').mask('0000,00', {reverse: true});
      ...
      Shadowbox.init();
        ...
        if (confirm("Tem certeza que quer alterar para " + msg + "?"))
        {
            ...
            url = "<?php echo base_url() . 'admin/planos/alterar_periodo_status' ?>";
            $.ajax({
                type: "POST",
                url: url,
