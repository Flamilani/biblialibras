 ...
 <?php $q = $this->db->query("SELECT *, c.titulo as tema, cp.capitulo as cap FROM videos c INNER JOIN livros l ON l.id_livro = c.id_livro INNER JOIN capitulo cp on cp.id_capitulo = c.id_capitulo WHERE md5(c.id_livro) = '{$livroUrl}' AND md5(c.id_video) = '{$capUrl}' AND c.status = 1"); ?>
    <?php foreach($q->result() as $dado): ?>
      <div class="front">
          <h3><a href="<?php echo base_url('area'); ?>">Livros</a> > <?php echo $dado->titulo; ?> <span style="float: right;">Cap. <?php echo $dado->cap; ?></span> </h3>
          </div>
                   <?php endforeach; ?>
...
<div class="over">
    ...
    <?php $exibe_videos = $this->db->query("SELECT * FROM livros l INNER JOIN videos v on l.id_livro = v.id_livro
WHERE md5(l.id_livro) = '{$livroUrl}' AND v.id_capitulo =  {$titulo->id_capitulo} AND l.status = 1"); ?>
       <?php foreach ($exibe_videos->result() as $video): ?>
   <a class="list-group-item <?php echo md5($video->id_video) == $capUrl ? 'active' : '' ?>" href="<?php echo base_url('area/livro/' . md5($video->id_livro) . '/' . md5($video->id_video)); ?>" >
