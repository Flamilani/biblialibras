...
class Privacidade extends CI_Controller {
    ...
    public function index() {
        ...
        $data['privacidade'] = $this->privacidade_model->admin_Privacidade();
    ...
    public function gravarPrivacidade() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->privacidade_model->adicionarPrivacidade($titulo, $conteudo, $status)) {
                $this->session->set_flashdata("success", "A página Privacidade foi atualizada com sucesso.");
                redirect(base_url('admin/privacidade'));
            } else {
    ...
    public function gravar_alteracoes() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $id = $this->input->post('id_privacidade');
            ...
            if ($this->privacidade_model->gravar_alteracoes($id, $titulo, $conteudo, $status)) {
                $this->session->set_flashdata("success", "A página Privacidade foi atualizada com sucesso.");
                redirect(base_url('admin/privacidade'));
            } else {
                echo "Erro ao cadastrar a página";
